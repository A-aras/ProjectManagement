...
import {
  BsDatepickerModule
} from "ngx-bootstrap/datepicker/bs-datepicker.module";
import {
  DatePickerComponent
} from "ngx-bootstrap/datepicker/datepicker.component";
...
import {
  ValidationHelper
} from "src/app/validators/validation-helper";
import {
  PmServiceBus
} from "src/app/service/service_bus";
import {
  ProjectModel
} from "src/app/model/project-model";
import * as Validator from "src/app/validators/";
import * as Const from "src/app/const/const";
import {
  DatepickerModule
} from "ngx-bootstrap/datepicker/datepicker.module";
import {
  createDate
} from "ngx-bootstrap/chronos/create/date-from-array";
import {
  getTomorrowDate,
  getCurrentDate
  ...
  initFormsControl(): void {
    ...
    this.isParentTaskControl.valueChanges.subscribe(x => {
      if (x as boolean === false) {
        ...
        this.taskPriorityControl.setValidators([Validators.required,
          Validators.min(Const.priorityMin),
          Validators.max(Const.priorityMax)
        ]);
  ...
  HandleRowSelected(selectedValue: [any, boolean, string]) {
    if (selectedValue[1]) {
      if (selectedValue[2] === "ParentTask") {
        ...
        this.parentTaskControl.setValue(this.selectedParentTask.TaskDescription);
      }
